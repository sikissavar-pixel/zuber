...
export function LiveFeedCard({ reservation: r, onAccept, index = 0, variant = "timeline" }: LiveFeedCardProps) {
  const isCarousel = variant === "carousel";

  return (
    <motion.div
      initial={{ opacity: 0, x: isCarousel ? 0 : -30, scale: 0.95 }}
      animate={{ opacity: 1, x: 0, scale: 1 }}
      exit={{ opacity: 0, x: isCarousel ? 0 : 30, scale: 0.95 }}
      transition={{ duration: 0.5, delay: index * 0.1, ease: [0.25, 0.46, 0.45, 0.94] }}
      layout
      className={isCarousel ? "min-w-[320px] snap-start" : "group relative"}
    >
      {!isCarousel && (
      ...
      <div
        className={`relative ${isCarousel ? "ml-0" : "ml-12"} bg-gradient-to-br from-[#0d0d0d]/95 via-[#111]/90 to-[#0a0a0a]/95 border border-[#ffb400]/25 rounded-2xl p-5 backdrop-blur-xl hover:border-[#ffb400]/50 hover:shadow-[0_0_30px_rgba(255,180,0,0.15)] transition-all duration-500`}
      >
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-cinzel text-lg text-[#f5f5f5]">{r.guest_name || "Misafir"}</span>
            </div>
          </div>
        ...
        <div className="relative z-10 space-y-3 mb-5">
          ...
          {!isCarousel && <div className="ml-1.5 w-px h-4 bg-gradient-to-b from-[#ffb400] to-[#ffb400]/30" />}
        ...
        <div className="relative z-10 flex items-center justify-between pt-4 border-t border-[#ffb400]/10">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="relative overflow-hidden bg-gradient-to-r from-[#ffb400] to-[#ffcc33] hover:from-[#ffcc33] hover:to-[#ffd966] text-black font-bold py-3 px-6 rounded-xl shadow-[0_0_20px_rgba(255,180,0,0.3)] hover:shadow-[0_0_30px_rgba(255,180,0,0.5)] transition-all duration-300 flex items-center gap-2"
          >
            <span>Kabul Et</span>
